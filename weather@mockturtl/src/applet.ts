...
class MyApplet extends Applet.TextIconApplet {
  ...
  _icon_type: imports.gi.St.IconType;
  ...
  private provider: DataService; // API
  ...
  private OnSeparatorAreaRepaint(area: any) {
  ...
  private async RefreshWeather(rebuild: boolean): Promise < void > {
    ...
    try {
      ...
      if (!this.VerifyLocation()) return;

      this.InitWeatherProvider();

      let refreshResult = await this.provider.GetWeather();
      ...
      if (rebuild) this.RebuildPopupMenu();
      if (this.DisplayWeather() && this.DisplayForecasts()) {
  ...
  private InitWeatherProvider(): void {
    switch (this._dataService) {
      case DATA_SERVICE.DARK_SKY: // No city and country info
        if (!darkSky) darkSky = require('./darkSky');
        if (!this.provider || !(this.provider instanceof darkSky.DarkSky)) this.provider  = new darkSky.DarkSky(this);
        break;
      case DATA_SERVICE.OPEN_WEATHER_MAP: // No TZ info
        if (!openWeatherMap) openWeatherMap = require('./openWeatherMap');
        if (!this.provider || !(this.provider instanceof openWeatherMap.OpenWeatherMap)) this.provider  = new openWeatherMap.OpenWeatherMap(this);
        break;
      default:
        return;
    }
  ...
  private async VerifyLocation(): Promise<boolean> {
    if (!this._manualLocation) { // Autmatic location
      ...
      if (!haveLocation) {
        this.log.Error("Couldn't obtain location");
        this.lastUpdated = null;
        return false;
      }
      ...
      if (this.weather.sunrise != null && this.weather.sunset != null) {
        if (this._showSunrise) {
          ...
          if (this.weather.location.timeZone != null) { //have TZ, en-GB returns time in the correct format
            let sunrise = this.weather.sunrise.toLocaleString("en-GB", {
              timeZone: this.weather.location.timeZone,
              hour: "2-digit",
              minute: "2-digit"
            });
            let sunset = this.weather.sunset.toLocaleString("en-GB", {
              timeZone: this.weather.location.timeZone,
              hour: "2-digit",
              minute: "2-digit"
            });
  ...
  private AddRefreshButton(): void {
    ...
    let refreshMenuItem = new PopupMenu.PopupIconMenuItem(itemLabel, REFRESH_ICON, Lang.bind(this, function () {
      this.RefreshWeather(false);
    }))
  ...
  /** Used on init */
  private BuildPopupMenuBase(): void {
    // build menu
    ...
    this._separatorArea.connect(SIGNAL_REPAINT, Lang.bind(this, this.OnSeparatorAreaRepaint))
  ...
  RebuildPopupMenu(): void {
    this.ShowLoadingUi()
    this.BuildCurrentWeatherUi()
    this.BuildFutureWeatherUi()
  }

  private DestroyCurrentWeatherUI(): void {
    if (this._currentWeather.get_child() != null)
      this._currentWeather.get_child().destroy()
  }

  private DestroyFutureWeatherUI(): void {
    if (this._futureWeather.get_child() != null)
      this._futureWeather.get_child().destroy()
  }

  private ShowLoadingUi(): void {
    this.DestroyCurrentWeatherUI()
    this.DestroyFutureWeatherUI()
  ...
  BuildFutureWeatherUi(): void {
    ...
    for (let i = 0; i < this._forecastDays; i++) {
      let forecastWeather: ForecastElement = {
        Icon: new St.Icon,
        Day: new St.Label,
        ...
        if (message) {
          try {
            if (message.status_code != 200) {
              reject({code: message.status_code, reason: message.reason_phrase});
              return;
            }
            ...
            reject({code: 1000, reason: e});
          }
        } else { // No response
          this.log.Error("Error: No Response from API");
          reject({code: 0, reason: "No response"});
        }
      });
    });
    return json;
